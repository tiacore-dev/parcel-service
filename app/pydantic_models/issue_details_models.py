...
class IssueDetailsCreateSchema(BaseModel):
    issue_id: UUID
    parcel_id: UUID
...
class IssueDetailsEditSchema(BaseModel):
...
class IssueDetailsResponseSchema(BaseModel):
    details_id: UUID
...
class IssueDetailsListResponseSchema(BaseModel):
    total: int
    details: List[IssueDetailsSchema]
...
def issue_details_filter_params(
    parcel_name: Optional[str] = Query(None, description="Фильтр по номеру накладной"),
    issue_id: Optional[UUID] = Query(None, description="Фильтр по ID транзита"),
