...
from app.database.models import DeliveryToRecipient


@pytest.mark.asyncio
async def test_add_delivery_to_recipient(
    ...
    response = await test_app.post(
        "/api/delivery-to-recipient/add", headers=headers, json=data
    )
    ...
    delivery = await DeliveryToRecipient.get_or_none(
        id=response_data["delivery_id"]
...
async def test_edit_delivery_to_recipient(
    ...
    updated_delivery = await DeliveryToRecipient.get_or_none(
        id=seed_delivery_to_recipient.id
    )
...
async def test_view_delivery_to_recipient(
    ...
    response = await test_app.get(
        f"/api/delivery-to-recipient/{seed_delivery_to_recipient.id}", headers=headers
    )
...
async def test_get_delivery_to_recipient_list(
    ...
    assert any(
        delivery["delivery_id"] == str(seed_delivery_to_recipient.id)
