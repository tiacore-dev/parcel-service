...
async def test_add_parcel_cargo(
    test_app: AsyncClient, jwt_token_admin, seed_parcel: Parcel, seed_cargo_type: CargoType
):
    ...
    data = {
        ...
        "height": "1.8",
        "quantity": "2",
        "cargo_type_id": str(seed_cargo_type.id),
    ...
    cargo = await ParcelCargo.get_or_none(id=response_data["cargo_id"]).prefetch_related("parcel")
...
@pytest.mark.asyncio
async def test_edit_parcel_cargo(test_app: AsyncClient, jwt_token_admin, seed_parcel_cargo: ParcelCargo):
    headers = {"Authorization": f"Bearer {jwt_token_admin['access_token']}"}
    data = {
    ...
    updated_cargo = await ParcelCargo.get_or_none(id=seed_parcel_cargo.id).prefetch_related("parcel")
    assert updated_cargo is not None
    assert updated_cargo.comment == "Обновлённый комментарий"


@pytest.mark.asyncio
async def test_view_parcel_cargo(test_app: AsyncClient, jwt_token_admin, seed_parcel_cargo: ParcelCargo):
    headers = {"Authorization": f"Bearer {jwt_token_admin['access_token']}"}

    response = await test_app.get(f"/api/parcel-cargo/{seed_parcel_cargo.id}", headers=headers)
...
@pytest.mark.asyncio
async def test_delete_parcel_cargo(test_app: AsyncClient, jwt_token_admin, seed_parcel_cargo: ParcelCargo):
    headers = {"Authorization": f"Bearer {jwt_token_admin['access_token']}"}

    response = await test_app.delete(f"/api/parcel-cargo/{seed_parcel_cargo.id}", headers=headers)
