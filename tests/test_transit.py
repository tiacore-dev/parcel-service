...
@pytest.mark.asyncio
async def test_edit_transit(
    test_app: AsyncClient, jwt_token_admin, seed_transit: Transit
):
    headers = {"Authorization": f"Bearer {jwt_token_admin['access_token']}"}
    ...
    data = {
        "warehouse_to_id": str(new_warehouse_to_id),
    }
